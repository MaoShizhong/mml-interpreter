...
describe('checking only mml strings', (): void => {
    ...
    describe('tokenizing durations with dots', (): void => {
        ...
        it('throws RangeError if dot not attached to a duration', (): void => {
            ...
            expect((): string[] => toMmlStringArray('B4.C..')).toThrow(
                DotError
            );
...
describe('checking full token details', (): void => {
    ...
    test('changing modifiers does not affect preceding tokens', (): void => {
        const STARTING_MODIFIERS = { tempo: 100, lengthOfNote: 4 };
        const CHANGED_MODIFIERS = { tempo: 200, lengthOfNote: 8 };
        const [firstNote, secondNote] = parse(
            `AT${CHANGED_MODIFIERS.tempo}BL${CHANGED_MODIFIERS.lengthOfNote}C`,
            {
        ...
        expect(firstNote.details.lengthInCrotchets).toBe(
            CHANGED_MODIFIERS.lengthOfNote
        );
        expect(firstNote.details.referenceBpm).toBe(CHANGED_MODIFIERS.tempo);
